...
# Show APK Files Comparision text in center
clear
echo ""
COLUMNS=$(tput cols)
title="APK Files Comparision"
        ...
        if command -v apktool >/dev/null 2>&1 && command -v diff >/dev/null 2>&1; then
            ...
            if [ $choice -eq 1 ]; then
                ...
                if [ -z "$output" ]; then
                    ...
                    echo ""
                    echo "$output" | tee apk_resources_changes_$log_file
                    echo ""
                ...
                if [ -z "$output" ]; then
                    ...
                    echo ""
                    echo "$output" | tee apk_smali_changes_$log_file
                    echo ""
                    echo -e "${GREEN}The comparison result's logs are saved. You can find them here:${NC}"
                    echo "$dir"
                    echo ""
                ...
                if [ -z "$output" ]; then
                    ...
                    echo ""
                    echo "$output" | tee apk_changes_$log_file
                    echo ""
                    echo -e "${GREEN}The comparison result's logs are saved. You can find them here:${NC}"
                    echo "$dir"
                    echo ""
